---
title: "European Homicides and danish monarchs connection"
author: "Emma Kirkegaard Holst Hansen"
date: "`r paste0('created 19 marts 2026, ', format(Sys.time(), '%d %B, %Y at %H:%M'))`"
output: 
    html_document:
    toc: true
    toc_float: true
...
Western_Europe %>% 
  ggplot(aes(x=Year, y=Homicide_per_100k, color=Countries))+
  geom_line()+
  theme_minimal()+
...
Western_Europe %>% 
  ...
  facet_wrap(~Countries, ncol=2, nrow=3)
...
There could be a connection between the stability of the monarchy, how long the monarch is reigning, and the amount of murders occuring. 

```{r eval = TRUE}
# YOUR CODE HERE:
kings <- read_csv2("data/Den danske kongerække-kopi.csv")
head(kings) 
kings %>% 
  ...
  geom_smooth()+
  labs(title = "Tendens i danske monarkers regeringstid over tid",
       x = "Midtpunkt for regeringstid (År)",
       y = "Varighed af regeringstid (Antal år)") +
  theme_minimal()
```
...
The visualization of Danish monarchs reveals a significant trend that is that the duration of reigns has generally increased over time. By plotting the midyear of each ruler's reign against its duration using geom_smooth(), a clear image is created, indicating that political stability has strengthened through the centuries. When comparing this to historical homicide trends—which typically show a dramatic decline in interpersonal violence since the Middle Ages, a  picture of societal development shows. In earlier periods, shorter reign durations often connected with high levels of societal voiolence struggles. In contrast, the modern era is characterized by both longer individual tenures for rulers and a significant reduction in the frequency of homicide within the general population. Based on these data visualizations, we can articulate that we are more "civilized" today, provided that civilization is measured by the reduction of physical violence and the increase in institutional stability. The data suggests that the transition from a society where power was often settled through lethal conflict to one governed by stable, long-term rule has directly correlated with a safer environment for the average citizen. This synchronization of increasing reign duration and decreasing violence indicates a successful "civilizing process," where the state’s ability to maintain order has effectively lowered the risks to human life over time.
