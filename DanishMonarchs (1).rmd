...
library(tidyverse)
kings1 <- read.csv("data/MyKings.csv")

kings2 <- read_csv("data/MyKings.csv")

kings3 <- read.csv2("data/MyKings.csv")

kings4 <- read_csv2("data/MyKings.csv")
head(kings4)
```
Answer: 
1. Which of these functions is a `tidyverse` function? Read data with it below into a `kings` object

Read_csv and read_csv2 - I use read_csv2

2. What is the result of running `class()` on the `kings` object created with a tidyverse function.

Explains that it is a tibble and dataframe = "spec_tbl_df" "tbl_df"      "tbl"         "data.frame"
...
kings <- read_csv2("data/MyKings.csv")
...
```{r}
kings %>% 
  summarise(mean_reing=mean(duration,na.rm=TRUE))
```
